...
export default function SqlTool(){
 ...
 return <div className="flex h-full flex-col"><div className="flex flex-wrap items-center justify-between gap-2 border-b border-[var(--border)] px-4 py-2.5"><div className="flex items-center gap-2 text-sm"><Database className="size-4 text-[var(--muted-foreground)]"/><span className="font-medium">SQL Formatter</span></div><div className="flex gap-2"><Button size="sm" variant={mode==="format"?"default":"secondary"} onClick={()=>setMode("format")}><Wand2 className="size-3.5"/> Format</Button><Button size="sm" variant={mode==="minify"?"default":"secondary"} onClick={()=>setMode("minify")}><Minimize2 className="size-3.5"/> Minify</Button><Button size="sm" variant="ghost" onClick={()=>setInput("")} disabled={!input}><Eraser className="size-3.5"/> Xóa</Button></div></div>
 <div className="grid flex-1 grid-cols-1 overflow-hidden md:grid-cols-2"><div className="flex flex-col overflow-hidden border-b border-[var(--border)] md:border-b-0 md:border-r"><Header label="Input"/><textarea value={input} onChange={(e)=>setInput(e.target.value)} spellCheck={false} className="min-h-0 flex-1 resize-none bg-transparent p-3 font-mono text-sm outline-none" placeholder="Paste SQL..."/></div><div className="flex flex-col overflow-hidden"><Header label="Output" right={<CopyButton text={output}/>}/><pre className="min-h-0 flex-1 overflow-auto whitespace-pre-wrap p-3 font-mono text-sm leading-relaxed">{highlight(output)}</pre></div></div></div>
}
