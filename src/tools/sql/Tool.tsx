...
export default function SqlTool(){
 ...
 return <div className="flex h-full flex-col"><div className="flex items-center justify-between gap-2 overflow-x-auto border-b border-[var(--border)] px-4 py-2.5"><div className="flex items-center gap-2 text-sm"><Database className="size-4 text-[var(--muted-foreground)]"/><span className="font-medium">SQL Formatter</span></div><div className="flex gap-2"><Button size="sm" variant={mode==="format"?"default":"secondary"} onClick={()=>setMode("format")}><Wand2 className="size-3.5"/> {t("action.format")}</Button><Button size="sm" variant={mode==="minify"?"default":"secondary"} onClick={()=>setMode("minify")}><Minimize2 className="size-3.5"/> {t("action.minify")}</Button><Button size="sm" variant="ghost" onClick={()=>setInput("")} disabled={!input}><Eraser className="size-3.5"/> {t("action.clear")}</Button></div></div>
