    ...
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between gap-2 overflow-x-auto border-b border-[var(--border)] px-4 py-2.5">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" size="sm" onClick={() => { setMessage(""); setSecret(""); }} disabled={!message && !secret}><Eraser className="size-3.5"/> {t("action.clear")}</Button>
        </div>
      </div>
      <div className="grid flex-1 grid-cols-1 overflow-hidden md:grid-cols-2">
        <div className="flex flex-col gap-3 overflow-auto border-b border-[var(--border)] p-4 md:border-b-0 md:border-r">
          <label className="flex flex-col gap-1.5"><span className="text-xs font-medium">{t("label.secretKey")}</span><input value={secret} onChange={(e)=>setSecret(e.target.value)} className="h-10 rounded-lg border border-[var(--border)] bg-[var(--muted)]/30 px-3 font-mono text-sm outline-none"/></label>
          <label className="flex min-h-0 flex-1 flex-col gap-1.5"><span className="text-xs font-medium">{t("tool.hmac.message")}</span><textarea value={message} onChange={(e)=>setMessage(e.target.value)} spellCheck={false} className="min-h-[260px] flex-1 resize-none rounded-lg border border-[var(--border)] bg-transparent p-3 font-mono text-sm outline-none"/></label>
        </div>
        <div className="flex flex-col gap-3 overflow-y-auto p-4">
          {ALGOS.map((a) => <Result key={a} label={`HMAC ${a}`} value={results[a]} />)}
          {sampleHeader && <Result label={t("tool.hmac.sampleHeader")} value={sampleHeader} />}
        </div>
      </div>
    </div>
  );
