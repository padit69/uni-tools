...
export default function CidrTool() {
  ...
  const result = useMemo(()=>parse(cidr),[cidr]);
  ...
  return <div className="flex h-full flex-col">
    <div className="flex items-center gap-2 border-b border-[var(--border)] px-4 py-2.5 text-sm"><Network className="size-4 text-[var(--muted-foreground)]"/><span className="font-medium">CIDR / IPv4 Calculator</span></div>
    <div className="flex flex-col gap-4 overflow-auto p-4">
      ...
      {!result.ok ? <div className="rounded-lg border border-red-500/30 bg-red-500/5 p-3 text-sm text-red-400">{result.error}</div> : <>
        ...
        <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
          ...
          <Row label="Broadcast" value={intToIp(result.broadcast)} />
          <Row label="First usable" value={intToIp(result.first)} />
          <Row label="Last usable" value={intToIp(result.last)} />
          <Row label="Total addresses" value={String(result.total)} />
          <Row label="Usable hosts" value={String(result.usable)} />
          <Row label="IP integer" value={String(result.ip)} />
        </div>
      </>}
    </div>
  </div>;
